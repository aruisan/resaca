...
@include('alertas.request')
    ...
    {!! Form::model($reservas, [ 'route' => ['reservaElementos.update', $reservas], 'method' => 'PUT']) !!}
            ...
            @endforeach
             ...
             <div class="col-md-5"> 
                {!! Form::label('elemento', 'Escoge el Elemento:', ['for' => 'aula'] ) !!}                
                   <select name="elemento" id="seleccion" class="form-control select">
                @foreach($elementos as $elemento)
                    <option value="{{ $elemento->id }}" @if($reservas->elemento_id == $elemento->id)selected="selected"@endif >{{ $elemento->nombre}}</option>
                @endforeach
                    </select>
            </div>
            <div class="col-md-3"> 
              <label class="label-control">Cantidad</label>
                <div class="input-group">
                  ...
                  <input name="cantidad" type="text" class="btn form-control" value="{{$reservas->cantidad}}"/>
                </div>
            </div>
        </div>
